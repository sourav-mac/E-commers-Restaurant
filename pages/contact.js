...
export default function Contact(){
  ...
  const handleWhatsAppSubmit = (e) => {
    ...
    const whatsappMessage = `Hello Petuk 🍽️
  ...
  return (
    <div>
      ...
      <main className="container py-6 md:py-8">
        <h1 className="text-2xl md:text-3xl font-bold text-[var(--petuk-orange)]">Contact & Location</h1>
        <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <div className="card">
            <div className="font-semibold text-base md:text-lg">Address</div>
            ...
            <div className="mt-4 md:mt-6">
              <h4 className="font-semibold text-sm md:text-base">Opening hours</h4>
              <div className="text-xs md:text-sm text-gray-400 mt-1">Opens at {openingTime} AM — check live status on homepage</div>
            </div>
          </div>

          <div className="card">
            <h4 className="font-semibold text-base md:text-lg">Send us a message</h4>
            {submitted && (
              <div className="bg-green-900/30 border border-green-500/50 rounded p-3 mb-4 text-green-400 text-sm">
                ✅ Redirecting to WhatsApp... 📱
              </div>
            )}
            <form onSubmit={handleWhatsAppSubmit} className="mt-4 space-y-3 md:space-y-2">
              <input 
                name="name" 
                placeholder="Name" 
                value={formData.name}
                onChange={handleInputChange}
                className="w-full bg-[var(--petuk-dark)] border border-[var(--petuk-orange)]/30 rounded p-3 md:p-2 text-sm md:text-base text-[var(--petuk-offwhite)] placeholder-[var(--petuk-offwhite)]/40 focus:border-[var(--petuk-orange)] focus:outline-none transition"
                required
              />
              <textarea 
                name="message" 
